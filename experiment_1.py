...
def experiment(N_TRIALS):
	...
	for i_trial in range(N_TRIALS):
		...
		if key == None:
			...
			if  trial.get_factor('shape') == "square": #determine correct condition
				...
				if trial.get_factor('number_of_circles') == 3: # determine corgruence condition
					is_congruent = True #determine congruency
				elif trial.get_factor('number_of_circles') == 2: #alternative congruency condition
					is_congruent = False #alternative congruency
			elif trial.get_factor('shape') == "diamond":
				is_correct = (key == DIAMOND_RESPONSE_KEY)
				correct_key = 'S'
				if trial.get_factor('number_of_circles') == 2:
		...
		if  is_correct == False:
			...
			blankscreen.present()
			exp_1.clock.wait(PAUSE)
		exp_1.data.add([sex, i_trial, trial.get_factor('task'), trial.get_factor('shape'), trial.get_factor('number_of_circles'), is_congruent, correct_key, rt, is_correct])
...
exp_1.data_variable_names = ["Sex", "Trial Number", "Task Type","Frame Shape", "Number of Circles","Congruent Trial", "Correct Key", "RT", "Correct Response"]
